...
def main():
    ...
    df_codes = df_0XXR[["idn", "code"]].groupby(by="code").count().sort_values("idn", ascending=False)
    ...
    df = pd.read_csv("stats/gnd_systematik.csv", low_memory=False, names=["id", "count"])
    names = pd.read_csv("data/gnd_systematik_names.csv", low_memory=False, names=["id", "name"])
    ...
    df = pd.read_csv("stats/gnd_systematik_Ts.csv", low_memory=False, names=["id", "count"])
